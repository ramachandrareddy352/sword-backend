...
async function sendAdminOtp(req, res) {
    try {
        ...
        const adminConfig = await client_ts_1.default.adminConfig.findUnique({
            where: { id: 1 },
        });
        ...
        await (0, generateOTP_ts_1.sendEmail)(email, "Admin login OTP", `Admin login OTP code is: ${otp}. It expires in 15 minutes. Please make it confidential.`);
        // Save OTP (15 min)
        await redis_ts_1.default.set(`admin:otp:${email}`, otp, { EX: 900 });
        // Reset attempts for new OTP
        await redis_ts_1.default.set(`admin:otp:attempts:${email}`, "0", { EX: 900 });
        res.json({
            success: true,
...
async function verifyAdminOtp(req, res) {
    try {
        ...
        const savedOtp = await redis_ts_1.default.get(otpKey);
        if (!savedOtp) {
            return res.status(400).json({
                success: false,
        ...
        const attempts = Number(await redis_ts_1.default.get(attemptsKey)) || 0;
        // Max attempts reached
        if (attempts >= 5) {
            await redis_ts_1.default.del(otpKey);
            await redis_ts_1.default.set(attemptsKey, "0");
        ...
        if (savedOtp !== otp) {
            ...
            await redis_ts_1.default.set(attemptsKey, newAttempts.toString(), { EX: 900 });
            return res.status(400).json({
                success: false,
        ...
        await redis_ts_1.default.set(`admin:session:${jti}`, email, { EX: 60 * 60 * 2 });
        // Cleanup
        await redis_ts_1.default.del(otpKey);
        await redis_ts_1.default.del(attemptsKey);
        res.json({
            success: true,
...
async function adminLogout(req, res) {
    try {
        ...
        const existed = await redis_ts_1.default.exists(sessionKey);
        if (existed) {
            await redis_ts_1.default.del(sessionKey);
            return res.status(200).json({
                success: true,
