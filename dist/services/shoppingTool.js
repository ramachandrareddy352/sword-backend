...
export async function sendShoppingAck(payload) {
    const encoded = encodePayload(payload);
    const resp = await axios.post(process.env.SHOPPING_ACK_URL, { data: encoded }, { timeout: 5000 });
    if (!resp.data?.success) {
        throw new Error("Shopping acknowledge failed");
    }
    return resp.data;
}
export function classifyUserIdentifier(rawId) {
