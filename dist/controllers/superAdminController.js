...
export async function createAdmin(req, res) {
    try {
        ...
        if (config && config.adminEmailId.toLowerCase() === normalizedEmail) {
            return res.status(400).json({
                success: false,
                error: "This email is the super admin and cannot be added as a normal admin",
            });
        }
...
export async function updateAdmin(req, res) {
    try {
        ...
        return res.json({
            success: true,
            message: "Admin updated successfully. Their active sessions were revoked.",
...
async function revokeAdminSessions(email) {
    try {
