...
async function findSwordByLevel(tx, level) {
...
async function updateAdminConfig(req, res) {
    try {
        ...
        const validateUnsignedInt = (value, fieldName) => {
        ...
        const validateVersion = (value, fieldName) => {
            if (typeof value !== "string") {
                throw new Error(`${fieldName} must be a string`);
            }
            const parts = value.split(".");
            if (parts.length !== 3 || parts.some((p) => !/^\d+$/.test(p))) {
                throw new Error(`Invalid ${fieldName} format. Expected: major.minor.patch (e.g., "1.2.3")`);
            }
        };
        // ================= Shield Config =================
        ...
        if (data.shieldActiveOnMarketplace !== undefined) {
            updateData.shieldActiveOnMarketplace = Boolean(data.shieldActiveOnMarketplace);
        }
        // ================= Sword & Gold Ads =================
        ...
        if (updateData.minRequiredVersion &&
        ...
        if (data.mandatoryUpdateMessage !== undefined) {
            if (typeof data.mandatoryUpdateMessage !== "string" ||
                data.mandatoryUpdateMessage.trim().length < 10) {
                return res.status(400).json({
                    success: false,
                    error: "mandatoryUpdateMessage must be a string with at least 10 characters",
                });
            }
            updateData.mandatoryUpdateMessage = data.mandatoryUpdateMessage.trim();
        }
        if (data.notificationUpdateMessage !== undefined) {
            if (typeof data.notificationUpdateMessage !== "string" ||
                data.notificationUpdateMessage.trim().length < 5) {
                return res.status(400).json({
                    success: false,
                    error: "notificationUpdateMessage must be a string with at least 5 characters",
                });
            }
        ...
        if (data.playStoreLink !== undefined) {
            if (typeof data.playStoreLink !== "string" ||
        ...
        const status = err.message?.includes("Invalid") || err.message?.includes("cannot")
            ? 400
            : 500;
            ...
            try {
                created = await client_1.default.material.create({
                    data: {
                        ...
                        isBuyingAllow: isBuyingAllow !== undefined ? isBuyingAllow === "true" : true,
                        isSellingAllow: isSellingAllow !== undefined ? isSellingAllow === "true" : true,
                    },
                });
                break;
            }
    ...
    try {
        ...
        const updated = await client_1.default.material.update({
            ...
            data: {
                ...
                buyingCost: buyingCost !== undefined ? Number(buyingCost) : existing.buyingCost,
    ...
    try {
        ...
        if (typeof materialsRaw === "string" && materialsRaw.trim() !== "") {
            try {
                ...
                return res.status(400).json({
                    success: false,
                    error: "Invalid format for materials field – must be valid JSON array",
                });
            }
        }
        ...
        if (!name ||
            ...
            return res.status(400).json({
                success: false,
                error: "Name and synthesizeName are required and must be non-empty strings",
            });
        }
        ...
        if (isNaN(buy) ||
            ...
            return res.status(400).json({
                success: false,
                error: "Buying, selling, and upgrade costs must be positive numbers. Synthesize cost must be ≥ 0.",
            });
        }
        ...
        const sword = await client_1.default.$transaction(async (tx) => {
            const created = await tx.swordLevelDefinition.create({
                data: {
                    ...
                    isSynthesizeAllow: isSynthesizeAllow === "true" || isSynthesizeAllow === true,
                },
            });
...
async function updateSwordLevel(req, res) {
    try {
        ...
        if (buyingCost !== undefined)
            updateData.buyingCost = Number(buyingCost);
        if (sellingCost !== undefined)
            updateData.sellingCost = Number(sellingCost);
        if (upgradeCost !== undefined)
            updateData.upgradeCost = Number(upgradeCost);
        if (synthesizeCost !== undefined)
            updateData.synthesizeCost = Number(synthesizeCost);
        if (successRate !== undefined)
            updateData.successRate = Number(successRate);
...
async function updateSynthesizeRequirements(req, res) {
    try {
        ...
        const materialIds = materials.map((m) => BigInt(m.materialId));
        const uniqueIds = new Set(materialIds);
        if (uniqueIds.size !== materialIds.length) {
            return res.status(400).json({
                success: false,
        ...
        await client_1.default.$transaction(async (tx) => {
            ...
            const currentMaterialIds = new Set(currentRequirements.map((r) => r.materialId));
            // 3. Check that ALL provided materialIds are already attached
            const providedNotAttached = materialIds.filter((id) => !currentMaterialIds.has(id));
            ...
            const attachedNotProvided = [...currentMaterialIds].filter((id) => !materialIds.includes(id));
...
async function updateUpgradeDrops(req, res) {
    try {
        ...
        for (const m of materials) {
            ...
            if (typeof m.dropPercentage !== "number" ||
            ...
            if (typeof m.maxQuantity !== "number" ||
...
async function updateSwordMaterials(req, res) {
    try {
        ...
        for (const m of materials) {
            ...
            if (typeof m.requiredQuantity !== "number" ||
        ...
        if (missingIds.length > 0) {
            ...
            const missingNames = missingIds.map((id) => existingMaterials.find((m) => m.id === id)?.name || `ID ${id}`);
            return res.status(400).json({
                success: false,
