...
export const getAllSwords = async (req, res) => {
    try {
...
export const getSword = async (req, res) => {
    try {
...
export const getAllMaterials = async (req, res) => {
    try {
        const { rarity, sortBuyingCost, sortSellingCost, sortCreatedAt } = req.query;
        const pagination = getPagination(req.query);
        if (!pagination) {
            return res.status(400).json({
                success: false,
...
// 7) App Version Check
export const getAppVersionCheck = async (req, res) => {
    try {
