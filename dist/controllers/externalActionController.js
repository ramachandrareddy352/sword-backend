...
function resolveUserWhere(userId) {
...
export async function redeemVoucherFromShopping(req, res) {
    try {
        const { data } = req.body;
        if (!data) {
        ...
        try {
            ...
            const errorKey = err.message === "invalidTelegramUsername"
        ...
        const isAllowed = voucher.allowedUserId && voucher.allowedUserId === redeemUser.id;
        ...
        return res.status(500).json({
            success: false,
            error: err.message || req.t("externalAction.error.voucherRedemptionFailed"),
        });
    }
}
